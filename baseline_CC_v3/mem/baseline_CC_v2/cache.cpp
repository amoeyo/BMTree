...
Cache& g_dr_Cache = Cache::get_dr_instance();
#endif
...
#ifdef USING_EXTRA_CACHE_FOR_INDEX_TABLE
status read_dr_cache(void* addr, void* buffer, size_t n_bytes) {
	return g_dr_Cache.read(addr, buffer, n_bytes, nullptr, false);
}

status write_dr_cache(void* addr, void* buffer, size_t n_bytes, bool write_back = false) {
	return g_dr_Cache.write(addr, buffer, n_bytes, nullptr, write_back, false);
}

status clear_dr_cache() {
	return g_dr_Cache.clear_cache();
}
#endif